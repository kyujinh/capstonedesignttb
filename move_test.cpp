		...
		if(ball_number==0 )
		...
		else
		 {
			 ...
			 for(int i = 0; i < red_number-1; i++)
			 		{
			 			if(abs(ball_X_r[i])<0.1 && ball_distance[near_ball]>red_distance[i])
			 			{
			 				interrupt.push_back(i);
			 			}
			 //			printf("redballx%f\n",ball_X_r[i]);
			 	}
			 //	printf("interrupt?%lu\n",interrupt.size());
			 ...
			 //		printf("here?");
			 	if(abs(ball_X_r[near_red])<0.04 && red_distance[near_red]<0.04){
			 		avoid_red();
			 		printf("avoid");
			...
			if(abs(ball_X[near_ball])>0.03){
				if(ball_X[near_ball]  > 0){data[0]=20;data[1]=20;data[2]=20;data[3]=20;
					printf("turn left");
					printf(" ball_x%f\n",ball_X[near_ball]);
				}
				else{data[0]=-20;data[1]=-20;data[2]=-20;data[3]=-20;
						printf("turn right");
						printf(" ball_x%f\n",ball_X[near_ball]);
					}
